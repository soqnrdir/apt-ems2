...
<div class="p-3" @mousedown="closeToast">
  <div class="row">
    <div class="col-md-12">
      <h5 class="fw-bold text-start">미터기 관리</h5>
      <div class="card mt-4">
        <div class="card-body">
          <div class="row">
            <div class="text-end">
            ...
            <table class="table">
              ...
              <tbody v-if="table.data.length != 0">
                <tr v-for="(item, index) in paginatedData" :key="index" >
                  <td >{{item.meterRegNo}}</td>
                  <td style="color:#00BFFF;cursor:pointer" data-bs-toggle="modal" data-bs-target="#userUpdateModal" @click="userPopU(item)">{{item.metername}}</td>
                  <td>{{item.name}}</td>
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title fw-bold" id="userUpdateModalLabel">미터기 {{userPopTitle}}</h5>
            ...
            <form class="row g-3 needs-validation" novalidate @submit="clickSubmit" onsubmit="return false">
              <div class="modal-body text-start">
                ...
                <div class="mb-4">
                    <label for="metername-text" class="col-form-label"><span class="text-danger fs-3">*</span>미터기명:</label>
                    <input type="text" class="form-control" v-model="userData.metername" required id="metername-text" autocomplete="off">
                    <div class="invalid-feedback">
                      미터기명을 입력해주세요.
                    </div>
                </div>
                <div class="mb-4">
                    <label for="facilityId-text" class="col-form-label">계측대상:</label>
                    <select class="form-select" v-model="userData.facilityId" id="facilityId-text">
                      <option :key="i" :value="d.id" v-for="(d, i) in facilities">{{ d.name }}</option>
                    </select>
                </div>
                <div class="mb-4">
                    <label for="measurementId-text" class="col-form-label">계측기:</label>
                    <select class="form-select" v-model="userData.measurementId" id="measurementId-text">
                      <option :key="i" :value="d.id" v-for="(d, i) in measurements">{{ d.measureName }}</option>
                    </select>
                </div>
                <div class="mb-4">
                    ...
                    <select class="form-select" v-model="userData.type" id="type-text">
                      ...
                      <option value="Curoo">Curoo</option>
                    </select>
                </div>
                <div class="mb-4">
                    <label for="address-text" class="col-form-label">장치주소:</label>
                    <input type="text" class="form-control" v-model="userData.address" id="address-text" autocomplete="off">
                    <div class="invalid-feedback">
                      장치주소를 입력해주세요.
                    </div>
                </div>
        ...
        <div class="toast-container position-absolute top-0 end-0 p-3">
          <div class="toast" :class="display2" role="alert" aria-live="assertive" aria-atomic="true">
            <div class="toast-header">
              <strong class="me-auto">미터기 삭제</strong>
      ...
      <div class="toast-container position-absolute top-0 end-0 p-3">
        <div class="toast" :class="display" role="alert" aria-live="assertive" aria-atomic="true">
          <div class="toast-header">
            <strong class="me-auto">미터기 {{toastStats}}</strong>
...
<script>
const tableColumns = ['관리번호', '미터기명', '계측대상', '계측기', '장치주소' ,'타입', '비고']
  ...
  methods: {
    ...
    async getMeterList () {
      try {
        ...
        const res = await this.axios.get('/v1/meters/list-query')
    ...
    async getMeasurementList () {
      try {
        const res = await this.axios.get('/v1/measurement/all')
        this.measurements = res.data.records
      } catch (error) {
        console.log(error)
        alert('load error: ' + error.message)
      }
    ...
    async getFacilityList () {
      try {
        ...
        this.facilities = res.data.records
      } catch (error) {
        console.log(error)
        alert('load error: ' + error.message)
      }
    ...
    userPopR () {
      ...
      this.userData.regNo = ''
      this.userData.facilityId = this.facilities[0].id
      this.userData.measurementId = this.measurements[0].id
      this.userData.address = ''
      this.userData.note = ''
      this.userData.metername = ''
      this.userData.type = 'Egauge'
    },
    userPopU (item) {
      var forms = document.querySelectorAll('.needs-validation')
      Array.prototype.slice.call(forms)
      ...
      this.userData.regNo = item.meterRegNo
      this.userData.facilityId = item.facilityId
      this.userData.measurementId = item.measurementId
      this.userData.address = item.meterAddress
      this.userData.note = item.note
      this.userData.metername = item.metername
      this.userData.type = item.meterType
      this.id = item.id
    },
    clickSubmit () {
    ...
    async clickInsert () {
      try {
        ...
        await this.axios.post('/v1/meters/add', reqData)
        this.getMeterList()
    ...
    async clickUpdate () {
      try {
        const reqData = { ...this.userData }
        reqData.id = this.id
        await this.axios.put('/v1/meters/update', reqData)
        this.getMeterList()
    ...
    async deleteToast () {
      ...
      const id = this.id
      await this.axios.delete('/v1/meters/delete/' + id)
      this.getMeterList()
