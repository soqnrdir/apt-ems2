            ...
            <table class="table">
              ...
              <tbody v-if="table.data.length != 0">
                <tr v-for="(item, index) in paginatedData" :key="index" >
                  <td>{{item.regNo}}</td>
                  <td style="color:#00BFFF;cursor:pointer" data-bs-toggle="modal" data-bs-target="#userUpdateModal" @click="userPopU(item)">{{item.measureName}}</td>
                  <td>{{item.address}}</td>
                  <td>{{item.regDate}}</td>
          ...
          <div class="modal-content">
            ...
            <form class="row g-3 needs-validation" novalidate @submit="clickSubmit" onsubmit="return false">
              <div class="modal-body text-start">
                ...
                <div class="mb-4">
                    <label for="measureName-text" class="col-form-label"><span class="text-danger fs-3">*</span>계측기명:</label>
                    <input type="text" class="form-control" v-model="userData.measureName" id="measureName-text" required autocomplete="off">
                    <div class="invalid-feedback">
                      계측기명을 입력해주세요.
                    </div>
                </div>
                <div class="mb-4">
                    <label for="contype-text" class="col-form-label">연결타입:</label>
                    <select class="form-select" v-model="userData.contype" id="contype-text">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" class="form-control" v-model="userData.address" id="deviceAdd-text" autocomplete="off">
                    <div class="invalid-feedback">
                      장치주소를 입력해주세요.
                    </div>
                </div>
                ...
                <div class="mb-4">
                    <label for="modelName-text" class="col-form-label">모델명:</label>
                    <input type="text" class="form-control" v-model="userData.modelName" id="modelName-text"  autocomplete="off">
                    <div class="invalid-feedback">
                      모델명을 입력해주세요.
                    </div>
                </div>
...
<script>
const tableColumns = ['관리번호', '계측기명', '장치주소', '등록일', '모델명', '비고']
const tableData = [{
  controlNo: 'A-001',
  instrumentNm: '광선계측기',
...
export default {
  ...
  methods: {
    ...
    userPopR () {
      ...
      this.userData.regNo = ''
      this.userData.measureName = ''
      this.userData.contype = 'push'
      this.userData.address = ''
      this.userData.regDate = ref(new Date())
      this.userData.modelName = ''
      this.userData.note = ''
    },
    userPopU (item) {
      var forms = document.querySelectorAll('.needs-validation')
      ...
      this.userData.measureName = item.measureName
      this.userData.contype = item.contype
      this.userData.address = item.address
      this.userData.regDate = ref(new Date(item.regDate))
      this.userData.modelName = item.modelName
      this.userData.note = item.note
      this.id = item.id
    },
    clickSubmit () {
      if (this.userData.regNo !== '' && this.userData.measureName !== '') {
    ...
    async clickInsert () {
      try {
        ...
        await this.axios.post('/v1/measurement/add', reqData)
        this.getMeasurementList()
    ...
    async clickUpdate () {
      try {
        ...
        reqData.regDate = moment(reqData.regDate).format('YYYY-MM-DD')
        reqData.id = this.id
        await this.axios.put('/v1/measurement/update', reqData)
        this.getMeasurementList()
    ...
    async deleteToast () {
      ...
      const id = this.id
      await this.axios.delete('/v1/measurement/delete/' + id)
      this.getMeasurementList()
