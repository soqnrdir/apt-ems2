            ...
            <table class="table">
              ...
              <tbody v-if="table.data.length != 0">
                <tr v-for="(item, index) in paginatedData" :key="index" >
                  ...
                  <td style="color:#00BFFF;cursor:pointer" data-bs-toggle="modal" data-bs-target="#userUpdateModal" @click="userPopU(item)">{{item.sensorName}}</td>
                  <td>{{item.location}}</td>
                  <td>{{item.regDate}}</td>
          ...
          <div class="modal-content">
            ...
            <form class="row g-3 needs-validation" novalidate @submit="clickSubmit" onsubmit="return false">
              <div class="modal-body text-start">
                <div class="mb-3">
                    <label for="sensorType-text" class="col-form-label"><span class="text-danger fs-4">*</span>센서종류:</label>
                    <select class="form-select" v-model="userData.sensorType" id="sensorType-text">
                      <option value="미세먼지센서">미세먼지센서</option>
                      <option value="온습도센서">온습도센서</option>
                    </select>
                </div>
                <div class="mb-4">
                    <label for="sensorName-text" class="col-form-label"><span class="text-danger fs-4">*</span>센서명:</label>
                    <input type="text" :disabled="regNoDisabled" class="form-control" v-model="userData.sensorName" id="sensorName-text" required autocomplete="off">
                    <div class="invalid-feedback">
                      센서명을 입력해주세요.
                    </div>
                </div>
                <div class="mb-4">
                    <label for="location-text" class="col-form-label">설치위치:</label>
                ...
                <div class="mb-4">
                    <label for="ipAddress-text" class="col-form-label">IP주소:</label>
                    <input type="text" class="form-control" v-model="userData.ipAddress" id="ipAddress-text" autocomplete="off">
                    <div class="invalid-feedback">
                      ip주소를 입력해주세요.
                    </div>
                </div>
...
const tableColumns = ['센서종류', '센서명', '등록일', '설치날짜', 'IP주소', '장치주소', '비고']
export default {
  components: {
    Datepicker
  },
  ...
  methods: {
    ...
    userPopR () {
      ...
      this.userData.sensorType = '미세먼지센서'
      this.userData.sensorName = ''
    ...
    async clickInsert () {
      try {
        ...
        await this.axios.post('/v1/sensor/add', reqData)
        this.getSensorList()
    ...
    async clickUpdate () {
      try {
        ...
        reqData.regDate = moment(reqData.regDate).format('YYYY-MM-DD')
        reqData.id = this.id
        await this.axios.put('/v1/sensor/update', reqData)
        this.getSensorList()
    ...
    async deleteToast () {
      ...
      const id = this.id
      await this.axios.delete('/v1/sensor/delete/' + id)
      this.getSensorList()
