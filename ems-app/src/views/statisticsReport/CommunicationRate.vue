...
<div class="p-3">
  <div class="row">
    <div class="col-md-12">
      <h5 class="fw-bold text-start">데이터 통신율</h5>
      <div class="card mt-4">
        <div class="card-body">
          <div class="row">
            <div class="col-2">
              <select @change="meterChange" class="form-select" v-model="meterId" id="meter-text">
                  <option :key="i" :value="d.id" v-for="(d, i) in meters">{{ d.metername }}</option>
                </select>
            </div>
            <div class="col-2">
...
const tableColumns = ['일자','예상건수', '현재건수', '통신율[%]']
...
export default {
  ...
  methods: {
    ...
    async getUsageList () {
      ...
      try {
        const res = await this.axios.get(`/v1/usage/search-listPer?meterId=${this.meterId}&startDt=${startDt}&endDt=${endDt}`)
