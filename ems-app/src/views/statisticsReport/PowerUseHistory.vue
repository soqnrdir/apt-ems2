...
<div class="p-3">
  <div class="row">
    <div class="col-md-12">
      <h5 class="fw-bold text-start">미터기 데이터 이력</h5>
      <div class="card mt-4">
        <div class="card-body">
          <div class="row">
            <div class="col-2">
...
export default {
  ...
  methods: {
    ...
    async getUsageList () {
      ...
      try {
        const res = await this.axios.get(`/v1/usage/search-list?meterId=${this.meterId}&startDt=${startDt}&endDt=${endDt}`)
        this.table.data = res.data.records
        for (let i = 0; i < this.table.data.length; ++i) {
          this.table.data[i].time = moment(this.table.data[i].time).format('YYYY-MM-DD HH:mm:ss')
          this.table.data[i].energy = this.table.data[i].energy.toString()
          .replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");
        }
      } catch (error) {
        console.log(error)
